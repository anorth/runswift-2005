...
static const int MAX_NUM_BLOBS_ALL_COLORS = MAX_NUM_BLOBS * NUM_USABLE_COLORS;
...
class RunLengthInfo {
	public:
		...
		bool shouldJoin(RunLengthInfo *s) {
		  ...
		  if(this->color == cROBOT_BLUE || this->color == cROBOT_RED) {
		    start--; end++;
		  }
		  /*
		  if(this->row == 63 || this->row == 64) {
		    cout << "\t\tshouldJoin(";
		    cout << this->row << "," << s->row;
		    cout << "," << (this->color & 0xff) << "," << (s->color & 0xff); 

		    cout << "," << this->startIndex << "," << this->endIndex;
		    cout << "," << start << "," << end;

		    cout << "," << (this->startIndex <= end && this->endIndex >= start) << ")" << endl;
		  }
		  */
		  return (this->startIndex <= end && this->endIndex >= start);
		}
};
...
class eRunLengthInfo : public RunLengthInfo {
  ...
  void Set(int y, int s, int e, ColorList cl, bool edge)
 {
    ...
    colorLst.sort();
    isEdge = edge;
    RunLengthInfo::Set(y, s, e, colorLst.front().color);
  }
...
class BlobInfo {
	...
	public:
		...
		void Init(int bn, uchar rawColor, int y, int min, int max, long a, int xs, int ys) {
		  blob_number = bn;
		  color = rawColor & COLOR_MASK;
		...
		void Init(RunLengthInfo* s, int y) {
		  Init(s->blob_number, s->rawColor, 
		       y, s->startIndex, s->endIndex, s->length, 
		       s->xsum, s->ysum);
		}

		void Init(uchar myColor) {
			color = myColor;
		...
		*/
		void Update(uchar rawColor, int y, int min, int max, 
			    long a, int xs, int ys) {
		  // update NORTH
		  if (y == getYMin())
		...
		void Update(RunLengthInfo* s, int y) {
		  Update(s->rawColor, y, s->startIndex, s->endIndex, 
			 s->length, s->xsum, s->ysum);
		}
...
class Blobber {
	private:
		...
		HalfBeacon blobinfo[MAX_NUM_BLOBS_ALL_COLORS];
		...
		*/
		int top_unused_bn;
		int reusable_bn[MAX_NUM_BLOBS_ALL_COLORS];
		int top_reusable_bn;

		// creates a new blob, used by blob former
		void createBlob(int y, RunLengthInfo *segment);
		void Link(int y, RunLengthInfo *cs, RunLengthInfo *ps);
		void joinSegments(int y, RunLengthInfo *cs,RunLengthInfo *ps);
		void testSaveBlob(int y, RunLengthInfo *ps);
	...
	public:
		...
		void formDisjoint(SegmentedPlane& segmentPlane,bool ignoreBkg);
