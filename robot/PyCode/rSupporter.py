...
# We will bird to supporter pos if in front by this much
...
def resetEachFrame():
    ...
    #hFWHead.minPosVariance  = 35
    #hFWHead.minHeadVariance = 25
...
# Move to the supporter position, and return that target position
def supportBall(): 
    ...
    if distSquared <= hMath.SQUARE(gTargetPointReqAccuracy):
        ...
        if abs(ballH) < 5:
            Action.stopLegs()
            if 0<= Global.selfLoc.getHeading() <= 180 and not Global.vOGoal.isVisible():
                # only block if my heading is right and I cannot see my own goal
                sBlock.checkThenBlock(minBallSpeed = 2,
                                      bothSides = True)
    else:
        gTargetPointReqAccuracy = TARGET_PT_ACC_SMALL_CIRCLE 
...
# Using the rangeX, redetermine the supporter's targetX (to left or right
...
def getAdjustedTarget(targetX, targetY, rangeX):
    ...
    if Global.ballY > Constant.FIELD_LENGTH * 0.70:
...
# using the rangeX, redetermine the supporter's targetX by winger's position
