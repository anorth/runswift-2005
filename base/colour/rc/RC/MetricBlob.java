 ...
 * CHANGE LOG
 * + version 1.1:
 *       - add Interger <--> Float conversion, avoid floating hassles
 */

public class MetricBlob {
    //standard presion for all doubles
//    static protected final double PRECISION_FACTOR = 1e+2;

    int id;

    public MetricBlob(int num){
        id = num;
    }
    ...
    public static int toInteger(double d, double minimum, double maximum) {
        ...
        else if (d > maximum) d = maximum;

        return (int) Math.round( (d - minimum) / (maximum - minimum) * 100 );
    }
