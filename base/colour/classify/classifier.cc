...
void explore_neighbours(pixel* tset_p)
{
    ...
    if (tset_p->colour == ROBOT_GREY)
    {
        limit = 0;
    }
    else if (tset_p->colour == BLACK)
    {
        limit = 0;
    }
    else if (tset_p->colour == GREEN_FIELD)
    {
        limit = THRESHOLD-5; // ricky: was 4
    }
    ...
    /* ricky: seems a bit much,lets just use default
    ...
    for (int i=0; i<CUBE_SIZE; ++i)
    {
        for (int j=0; j<CUBE_SIZE; ++j)
        {
            for (int k=0; k<CUBE_SIZE; ++k)
            {
                ...
                if (mset[index].colour == NONE) {
                    mset[index].colour = ROBOT_GREY;
                }
/*                
