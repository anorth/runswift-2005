...
#
#### Kim : customized for our directory structures
...
{
    print " ! ! ! ! ! Not found .nosnap.elf file in current dir \n";
    $objDirectory = $excepObj;
    #Tailored specifically for rUNSWift code
    if ($excepObj eq "Wireless"){
        $objDirectory = "wireless";
    }
    if ($excepObj eq "actuatorControl"){
        $objDirectory = "actuatorControl";
        $excepObj = "actuator"
    }
#    if ($excepObj eq "Vision"){
#        $objDirectory = "vision";
#    }

    $objDir = "../../robot/".$objDirectory;
    print " - - - - > Looking into ".$objDir." dir...";
    chomp $objDir;
    ...
    {
        print "\nStill not found, Please specify the directory where $excepObj was made:\n";
        $objDir = <STDIN>;
        chomp $objDir;
